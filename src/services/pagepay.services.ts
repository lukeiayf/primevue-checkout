...
export class PagePayService {
	...
	public async postCustomer(customer:CustomerRequest){
		...
		try {
			const data = await fetch(url, { method: "POST", body: JSON.stringify({customer}) });
			if (data.ok) {
	...
	public async postPaymentInfo(paymentInfo:PaymentRequest){
		...
		try {
			const data = await fetch(url, { method: "POST", body: JSON.stringify({paymentInfo}) });
			if (data.ok) {
				return await data.json();
			} else {
				throw new Error("erro no status da requisição");
	...
	public async postAddress(customerId: number,address: AddressRequest){
		...
		try {
			const data = await fetch(url, { method: "POST", body: JSON.stringify({ address }) });
			if (data.ok) {
				return await data.json();
			} else {
				throw new Error("erro no status da requisição");
	...
	public async postCard(customerId: number, card: CardRequest){
		...
		try {
			const data = await fetch(url, { method: "POST", body: JSON.stringify({ card }) });
			if (data.ok) {
				return await data.json();
			} else {
				throw new Error("erro no status da requisição");
	...
	public async putAddress(customerId:number, adress:AddressRequest){
		...
		try {
			const data = await fetch(url, { method: "PUT", body: JSON.stringify({ adress }) });
			if (data.ok) {
				return await data.json();
			} else {
				throw new Error("erro no status da requisição");
	...
	public async postAuthCode(customerId:number){
		...
		try {
			const data = await fetch(url, { method: "POST", body: JSON.stringify({ customerId }) });
			if (data.ok) {
				return await data.json();
			} else {
				throw new Error("erro no status da requisição");
	...
	public async postVerifyAuthCode(customerId: number, code: string){
		...
		try {
			const data = await fetch(url, { method: "POST", body: JSON.stringify({ customerId, code }) });
			if (data.ok) {
				return await data.json();
			} else {
				throw new Error("erro no status da requisição");
