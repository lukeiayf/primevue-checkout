  ...
  <div class="flex-nowrap">
    <span class="p-float-label">
      <InputNumber v-model="v$.installments.$model" id="installments" showButtons mode="decimal" :min="1"
        ...
        :class="{ 'full input-size p-invalid': v$.installments.$invalid && props.submitted }" />
      <label for="number" :class="{ 'p-error': v$.installments.$invalid && props.submitted }">{{
  ...
  <div v-if="v$.paymentMethod.$model.value == 'CREDIT_CARD'" class="input-area" style="margin-top: 25px">
    <div>
      <span class="p-float-label" v-if="v$.cardNumber.$model.length > 13">
        ...
        <label for="number" :class="{ 'p-error': v.cardBrand.$invalid && props.submitted }">{{ $t('cartao.bandeira')
        }}*</label>
      </span>
      <small v-if="(v.cardBrand.$invalid && props.submitted) || v.cardBrand.$pending" class="p-error">{{
          $t('erros.cartao.bandeiraRequerida')
      }}</small>
    </div>
    <div>
      ...
      <small v-if="(v$.cardNumber.$invalid && props.submitted) || v$.cardNumber.$pending" class="p-error">{{
          $t('erros.cartao.numeroRequerido')
      }}</small>
    </div>
    <div class="flex-row">
      <div class="width-45">
        ...
        <small v-if="(v$.dueDate.$invalid && props.submitted) || v$.dueDate.$pending" class="p-error">{{
            $t('erros.cartao.validadeRequerida')
        }}</small>
      </div>
      <div class="width-45">
        <span class="p-float-label p-input-icon-right  full">
          ...
          <label for="number" :class="{ 'p-error': v$.securityCode.$invalid && props.submitted }">{{ $t('cartao.codigo')
          }}*</label>
        </span>
        <small v-if="(v$.securityCode.$invalid && props.submitted) || v$.securityCode.$pending" class="p-error">{{
            $t('erros.cartao.codigoRequerido')
        }}</small>
      </div>
    </div>
    <div>
      <span class="p-float-label p-input-icon-right full">
        ...
        <label for="number" :class="{ 'p-error': v$.holderName.$invalid && props.submitted }">{{
            $t('cartao.nomeTitular')
        }}*</label>
      </span>
      <small v-if="(v$.holderName.$invalid && props.submitted) || v$.holderName.$pending" class="p-error">{{
          $t('erros.cartao.nomeRequerido')
      }}</small>
    </div>
    <div>
      <span class="p-float-label p-input-icon-right full">
        ...
        <label for="number" :class="{ 'p-error': v$.holderDocument.$invalid && props.submitted }">{{
            $t('cartao.documentoTitular')
        }}*</label>
      </span>
      <small v-if="(v$.holderDocument.$invalid && props.submitted) || v$.holderDocument.$pending" class="p-error">{{
          $t('erros.cartao.documentoRequerido')
      }}</small>
    </div>
  </div>
</template>
